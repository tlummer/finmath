...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		System.out.println("The Chebyshev condifence interval boundaries at a " + confidenceLevel*100+ "% confidence level  for lambda  after " + numberOfExponentialDrawings+" drawings are \n" + 
				exponential.getParamterLowerBoundConfidenceInterval(confidenceLevel, cheb) + " and " + exponential.getParamterUpperBoundConfidenceInterval(confidenceLevel, cheb));
		System.out.println("\n");
		...
		System.out.println("The CLT condifence interval boundaries at a " + confidenceLevel*100+ "% confidence levelfor lambda after "  + numberOfExponentialDrawings+" drawings are \n" + 
				exponential.getParamterLowerBoundConfidenceInterval(confidenceLevel, clt) + " and " +  exponential.getParamterUpperBoundConfidenceInterval(confidenceLevel, clt));
		System.out.println("\n");
		...
		for(int i=0; i<numberOfSimulations; i++){
			...
			if(normalTestSampler.generateBivariateNormal().
					getFirstValue()<mu &&  normalTestSampler.generateBivariateNormal().getSecondValue()<mu){
					sum++;
			}
	}
